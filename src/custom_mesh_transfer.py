...
V1, F1 = igl.read_triangle_mesh(current_folder+"/../meshes/skinweight_test_data/body.obj")
# V1, F1,_,_ = igl.remove_unreferenced(V1,F1)
N1 = igl.per_vertex_normals(V1, F1)

# Load the target mesh
V2, F2 = igl.read_triangle_mesh(current_folder+"/../meshes/skinweight_test_data/cloth1.obj")
# V2, F2,_,_ = igl.remove_unreferenced(V2,F2)
N2 = igl.per_vertex_normals(V2, F2)

# Load the .npy file
W = np.load(current_folder+"/../meshes/skinweight_test_data/body_skinweight.npy")
...
num_bones = W.shape[1] # our simple rig has only 2 bones
# You can setup your own skin weights matrix W \in R^(|V1| x num_bones) here
# Generate simple per-vertex data (can be skinning weights but can be any scalar data)
# W = np.ones((V1.shape[0], num_bones))
# W[:, 0] = 0.3 # first bone has an influence of 0.3 on all vertices
...
#
InpaintedWeights,_ = inpaint(V2, F2, SkinWeights_interpolated, Matched)
